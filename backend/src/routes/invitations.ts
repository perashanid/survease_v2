    ...
    res.status(201).json({
      ...
      data: {
        invitation: {
          ...
          invitation_url: `${req.protocol}://${req.get('host')}/survey/${survey.slug}?token=${token}`
        }
      }
    });
  } catch (error: any) {
...
router.get('/:id/invitations', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const invitationsWithUrls = invitations.map(invitation => ({
      ...
      invitation_url: `${req.protocol}://${req.get('host')}/survey/${survey.slug}?token=${invitation.token}`,
