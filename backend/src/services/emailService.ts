...
// Load enhanced email template
const loadEnhancedTemplate = (): string => {
  try {
    const templatePath = path.join(__dirname, '../../email-template-enhanced.html');
    return fs.readFileSync(templatePath, 'utf-8');
  } catch (error) {
    console.error('❌ Failed to load enhanced email template:', error);
...
export class EmailService {
  ...
  private static async sendViaBrevoAPI(
    ...
    // Load and customize the enhanced template
    let htmlContent = loadEnhancedTemplate();
    
    // Replace placeholders in the template
    htmlContent = htmlContent
      .replace(/John Doe/g, displayName)
      .replace(/https:\/\/survease\.onrender\.com\/reset-password\?token=sample/g, resetUrl)
      .replace(/https:\/\/survease\.onrender\.com\/reset-password\?token=sample-token-12345/g, resetUrl);
    ...
    const emailData = {
      ...
      subject: 'Password Reset Request - SurvEase',
      htmlContent
    };
...
We received a request to reset your password for your SurvEase account.
